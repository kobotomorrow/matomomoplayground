...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    let token_url = TokenUrl::new("https://api.fitbit.com/oauth2/token".to_string())?;
    ...
    let (auth_url, _csrf_token) = client
        ...
        .add_scopes(vec![
            Scope::new("activity".to_string()),
            Scope::new("heartrate".to_string()),
        ])
    ...
    let rs_fitbit_dir = std::path::PathBuf::from(home).join(".config/rs_fitbit");
    std::fs::create_dir_all(&rs_fitbit_dir)?;
    
    let tokens_path = rs_fitbit_dir.join("tokens.json");
